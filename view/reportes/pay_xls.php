...
if($ctry != 0){
      ...
      G.int_CargosEmpresa,G.int_CargosAgente,G.int_Descuentos,G.int_TotalPagar,G.int_factura,G.int_CiudadDestinatario,G.int_ciudadRemitente,G.int_IDuser,G.int_CCEmpresa,G.int_CCAgente,G.int_TarEmpresa,G.int_TarAgente,G.int_SegEmpresa,G.int_SegAgente,G.int_ImpEmpresa,G.int_ImpAgente,G.int_Servicio,G.int_Pago,G.int_PayDate,ia.var_Pais,ia.int_Pais 
        FROM  tbl_Guia G   inner join tbl_Ciudades_iata ia on G.int_Pago = 1 AND G.int_Activo = '1' AND (ia.int_Pais = '".$ctry."' AND G.int_CiudadDestinatario = ia.int_ID) AND 
      G.date_FechaEnvio  BETWEEN $fecha $qr inner join tbl_Servicios srv on G.int_Servicio = srv.int_ID AND srv.int_cod = $sv ORDER BY G.date_FechaEnvio DESC"
 ...
 $resultList = mysqli_query
      ...
      FROM  tbl_Guia G inner join tbl_Ciudades_iata ia on G.int_Pago = 1 AND G.int_CiudadDestinatario = ia.int_ID inner join tbl_Servicios srv on  G.int_Activo = '1' AND G.date_FechaEnvio BETWEEN $fecha $qr AND G.int_Servicio = srv.int_ID AND srv.int_cod = $sv  ORDER BY G.date_FechaEnvio DESC"
...
foreach ($listItems as $key ) { 
	...
	$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue("D".$d, $key[29])
            ->setCellValue("E".$d, $key[30])
...
header('Content-Disposition: attachment;filename="ReportePagas.xls"');

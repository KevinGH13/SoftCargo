	...
	if(isset($_REQUEST['act'])){
		if($_REQUEST['act']=="add"){
			$vrcValoresCadena =
				...
				$vrcTex9 = $_REQUEST['txtCampo9'];if ($vrcTex9 == "on"){$vrcTex9 = 1;}else{$vrcTex9=0;}
				
				
		$addInfo =insertData(
			"tbl_Impuestos",
			"int_IDAgente, var_Servicio, var_Pais, int_porcEmpresa, int_porcAgente, int_PesoMin, int_PesoMax,
				int_ValorDeclaradoMin, var_Formula,int_opcional, int_Activo",
	...
	if(isset($_REQUEST['act'])){
		...
		$updInfo =updateData(
			...
			"int_IDAgente=$vrcTex1, var_Servicio='$vrcTex2', var_Pais='$vrcTex3', int_porcEmpresa=$vrcTex4,	int_porcAgente=$vrcTex5,
				int_PesoMin=$vrcTex6, int_PesoMax=$vrcTex7, int_ValorDeclaradoMin=$vrcTex8,int_opcional = $vrcTex9",
			$vrcID);

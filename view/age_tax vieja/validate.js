...
function abre_del(int_ID,int_IDAgente,int_IDTax,bool_del)
{
	var xmlhttp;
	if (window.XMLHttpRequest){// code for IE7+, Firefox, Chrome, Opera, Safari
	  xmlhttp=new XMLHttpRequest();
	...
	if (confirmar){
		...
		del_toast('showSuccessToast','registro eliminado');
		varVentana = "http://www.easycargopro.com/ECP/view/age_tax/index.php?id="+int_ID+"&id_Age="+int_IDAgente;
		setTimeout(function(){window.location.reload();},960);
	}
...
function exe_plus(int_ID,int_IDAgente,serc,pais){
	...
	if(confirmar){
		...
		varVentana = "http://www.easycargopro.com/ECP/view/age_tax/index.php?id="+int_ID+"&id_Age="+int_IDAgente+"&serc="+serc+"&pais="+pais;
		setTimeout(function(){window.open(varVentana,"_self");},960);
